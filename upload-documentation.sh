...
PUBLISH_SERVER="in.relation.to"
PUBLISH_SERVER_DIRECTORY="/var/www/docs.hibernate.org/"
PUBLISH_SERVER_HTTP_ADDRESS="https://docs.hibernate.org/"

# Push the documentation to the doc server
REMOTE_DIRECTORY="${PUBLISH_SERVER}:${PUBLISH_SERVER_DIRECTORY}"
echo "================================================================================"
...
if [ $REQUIRES_OUTDATED_CONTENT_UPDATE -eq 1 ]; then
  if [[ $RELEASE_VERSION =~ .*\.Final ]]; then
    wget -q "${PUBLISH_SERVER_HTTP_ADDRESS}_outdated-content/${PROJECT}.json" -O ${PROJECT}.json
    ...
    if [ "$CURRENT_STABLE_VERSION" != "$VERSION_FAMILY" ] && version_gt $VERSION_FAMILY $CURRENT_STABLE_VERSION; then
      ...
      # That's a bit overkill to rsync a single file... but it works!
      #  And matches the behaviour of syncing other documentation files:
      ...
      sftp ${PUBLISH_SERVER} -b <<EOF
cd ${PUBLISH_SERVER_DIRECTORY}stable
rm ${PROJECT}
